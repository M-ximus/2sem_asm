...
unsigned long Accumulator_for_average_ticks = 0;
...
size_t nanoogle(const char* way_to_dictionary, const char* way_to_requests, unsigned int (*func_for_calc_hash)(const char*))
{
    ...
    printf("%lu\n", Accumulator_for_average_ticks / num_requests);
...
List_pointer_Elem* Find_elem(my_pointer_List* List, const char* request)
{
    ...
    {
        unsigned long start_time = curr_tick_num();
        bool check = compare_req(current->data, request);
        sum_time = (curr_tick_num() - start_time) + sum_time;
...
bool compare_req(const char* str_for_comp, const char* request)
{
    ...
    int i = 1;
    while(str_for_comp[i] == request[i] && str_for_comp[i] != '\0' && str_for_comp[i] != ':' && request[i] != '\0')
    {
        i++;
    }
...
int searching_machine(my_pointer_List** hash_table, str* requests, unsigned int (*func_for_calc_hash)(const char*))
{
    ...
    while(requests[i].String != nullptr)
    {
        ...
        hash = (*func_for_calc_hash)(requests[i].String) % Size_hash_table;
...
void curr_tick_reg(unsigned int* high, unsigned int* low)
{
    asm (   "RDTSC\n\t"
            "movl %[H], %%edx\n\t"
            "movl %[L], %%eax\n\t"
